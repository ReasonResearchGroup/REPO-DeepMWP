...
class Evaluator(object):
    ...
    def retrieval_evaluate(self, model, test_data,postfix):
        ...
        for step, data in enumerate(test_data):
            ...
            #similarity, gen_temp, sentence = model.max_similarity(data['sentence'][0])
            ...
            for i in range(1):
                ...
                gen_equ = self.get_new_tempalte(seq_var[i], batch_num_list[i])
                target_ans = batch_solution[i]
                #gen_equ=self.inverse_temp_to_num_(gen_temp,batch_num_list[i])
                #tmp_equ=self.inverse_temp_to_num_(tmp_equ,batch_num_list[i])
                gen_equ = []
    ...
    def hybrid_evaluate(self, model, test_data, postfix):
        ...
        for step, data in enumerate(test_data):
            ...
            gen_temp = model(data['sentence'][0],input_variables,input_lengths,target_variables)
            seqlist = gen_temp
            seq_var = seqlist
            target_equ_right_flag = []
            seq_var=self._convert_f_e_2_d_sybmbol(seq_var)
